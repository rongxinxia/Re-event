...
const validate = combineValidators({
    newPassword1:isRequired({message:'Enter the Password'}),
    newPassword2:composeValidators(
      isRequired({message:'Enter the Password'}),
      matchesField('newPassword1')({message:'Passwords do not match'})
    )()
}
)

const AccountPage = ({ error,invalid,submitting,handleSubmit, updatePassword,provider}) => {
  return (
    <Segment>
      ...
      {provider && provider==='facebook.com' && <div>
      ...
      {provider && provider==='google.com' && <div>
